"""
Rig builder for the pure-Python TwistSpline nodes (Phase 3).

This is the Blur Studio ``twistSplineBuilder`` adapted to assemble a rig from the
``py*`` node types instead of the C++ ones. The two builders are line-for-line
identical except for the four ``createNode`` type names below -- possible because
the Python nodes mirror every attribute long-name of their C++ counterparts:

    twistSpline       -> pyTwistSpline
    twistTangent      -> pyTwistTangent
    twistMultiTangent -> pyTwistMultiTangent
    riderConstraint   -> pyRiderConstraint

Requires the Python plugin loaded (pyTwistSplinePlugin). Original (C++) builder
remains untouched at scripts/twistSplineBuilder.py for reference/comparison.
...
def _createTangentSegmentSetup(
    ...
    ott = cmds.createNode("pyTwistTangent", name="twistTangentOut")
    ...
    itt = cmds.createNode("pyTwistTangent", name="twistTangentIn")
...
def connectTwistSplineTangents(cvs, twBfrs, oTans, iTans, aoTans, aiTans, closed=False):
    ...
    for i in range(segments):
        ...
        twt = cmds.createNode("pyTwistTangent", name="twistAuto")
...
def connectTwistSplineMultiTangents(
    ...
    tmt = cmds.createNode("pyTwistMultiTangent", name="twistMultiAuto")
    if closed:
...
def buildTwistSpline(pfx, cvs, aoTans, aiTans, tws, maxParam, master, closed=False, twistMul=-1.0):
    ...
    spline = cmds.createNode("pyTwistSpline", parent=splineTfm, name=f"{SPLINE_FMT.format(pfx)}Shape")
...
def buildRiders(pfx, spline, master, numJoints, closed=False):
    ...
    cnst = cmds.createNode("pyRiderConstraint")
...
def makeTwistSpline(
    ...
    if not cmds.pluginInfo("pyTwistSplinePlugin", query=True, loaded=True):
        raise RuntimeError(
            "Load the Python plugin first: "
...
def convertToTwistSpline(pfx, crv, numJoints=10, singleTangentNode=True):
    ...
    tmpCnst = cmds.createNode("pyRiderConstraint")
